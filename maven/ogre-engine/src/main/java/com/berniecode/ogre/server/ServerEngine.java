...
/**
 * A {@link ServerEngine} exposes a number of {@link DataSource}s
 * 
 * TODO ServerEngine should also convert to OGRE Wire Format and cache messages
 * 
 * @author Bernie Sumption
 */
public class ServerEngine extends InitialisingBean implements GraphUpdateListener {
	...
	public void setDataSource(DataSource dataAdapter) {
		...
		setDataSources(new DataSource[] { dataAdapter });
	}
	 ...
	 * 
	 * <p>
	 * Must be called before {@link #initialise()}
	 */
	 ...
	 */
	@Override
	protected void doInitialise() throws OgreException {
		if (initialised) {
			return;
		}
		initialised = true;
	 ...
	 * 
	 * @throws NoSuchThingException if this {@link ServerEngineTest} does not manage the specified
	 *             type domain
	 */
	...
	public GraphUpdate getObjectGraph(String typeDomainId, String objectGraphId) throws NoSuchThingException {
		...
		throw new NoSuchThingException("This ServerEngineTest does not manage the object graph '" + typeDomainId + "/"
				+ objectGraphId + "'");
	}
